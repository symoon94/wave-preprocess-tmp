...
esc_path = os.path.abspath('/Users/sooyoungmoon/git/coretech/sampledata/t2_sample 2/' )
metadata_path = os.path.join(esc_path, 'meta/metadata.csv')
audio_path = os.path.join(esc_path, 'wav')
...
for index, row in metadata.iterrows():
    file_path = os.path.join(os.path.abspath(audio_path), str(row["file_name"]))
...
# X=np.array([np.array(each) for each in features])
...
# np.save("/Users/sooyoungmoon/git/coretech/ESC-50-master/X-mel_spec", X)
# np.save("/Users/sooyoungmoon/git/coretech/ESC-50-master/y-mel_spec", y)
